...
impl Action<RecvMsg> {
    ...
    pub fn poll_recv_from(
        ...
        let mut completion = ready!(Pin::new(&mut *self).poll(cx));
        let n = completion.result? as usize;
        buf[..n].copy_from_slice(&completion.action.buf[..n]);
        let addr = unsafe { to_socket_addr(completion.action.storage.as_mut_ptr() as *const _)? };
